...
describe('test suite: Applliance', () => {
    let applliance;

    beforeEach(() => {
        applliance = new Appliances({
    ...
    it('has the correct properties', () => {
        expect(applliance.id).toEqual('54e0eccd-8f36-462b-b68a-8182611d9add');
        expect(applliance.image).toEqual('images/products/black-2-slot-toaster.jpg');
        expect(applliance.instructionsLink).toEqual('images/appliance-instructions.png');
        expect(applliance.warrantyLink).toEqual('images/appliance-warranty.png');
    });

    it('gets the stars url', () => {
        expect(applliance.getStarsUrl()).toEqual('images/ratings/rating-50.png');
    });

    it('gets the price', () => {
        expect(applliance.getPrice()).toEqual('$18.99');
    });

    it('displays instructions and warranty in extraInfoHTML', () => {
        expect(applliance.extraInfoHTML()).toContain(`<a href="images/appliance-instructions.png" target="_blank">`);
        expect(applliance.extraInfoHTML()).toContain('Instructions');
        expect(applliance.extraInfoHTML()).toContain(`<a href="images/appliance-warranty.png" target="_blank">`);
        expect(applliance.extraInfoHTML()).toContain('Warranty');
    });
}); 